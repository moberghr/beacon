...
internal static class OidcEventHandlers
{
    public static async Task HandleTokenValidatedAsync(TokenValidatedContext context)
    {
        ...
        EnrichSemanticoClaims(context, user);
    }
    ...
    public static Task HandleRemoteFailureAsync(RemoteFailureContext context)
    {
        ...
        context.Response.Redirect("/semantico/login?ssoError=1");
        context.HandleResponse();
        return Task.CompletedTask;
    }

    private static void EnrichSemanticoClaims(TokenValidatedContext context, SemanticoUserData user)
    {
        ...
        ReplaceClaim(identity, SemanticoClaims.UserId, user.ExternalId);
        ReplaceClaim(identity, SemanticoClaims.UserName, user.UserName);
        ...
        foreach (var existingSemRole in identity.FindAll(SemanticoClaims.Role).ToList())
        {
        ...
        foreach (var role in user.Roles)
        {
            ...
            identity.AddClaim(new Claim(SemanticoClaims.Role, role.Name));
        }
