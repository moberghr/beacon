    ...
    <Dialog
      ...
      footer={
        <>
          ...
          <button type="submit" form="resolve-task-form" className="btn btn--primary" disabled={isSubmitting}>
    ...
    >
      <form id="resolve-task-form" onSubmit={onSubmit} noValidate>
        <div className="q-field">
          <label className="q-label" htmlFor="resolution-notes">Resolution notes</label>
          <textarea
            id="resolution-notes"
            className={`q-textarea${errors.resolutionNotes ? ' q-textarea--error' : ''}`}
            rows={5}
            placeholder="Describe how this issue was resolved (optional)…"
            {...register('resolutionNotes')}
          />
          {errors.resolutionNotes && <div className="q-error">{errors.resolutionNotes.message}</div>}
        </div>
      </form>
    </Dialog>
  );
