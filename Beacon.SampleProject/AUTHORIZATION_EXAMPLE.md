...
2. **Navigate to:** `https://localhost:7187/beacon`
...
The claims transformer adds Beacon-specific claims after authentication:
...
│ - Adds BeaconClaims.Role  │
│ - Adds user metadata         │
└────────┬─────────────────────┘
         │
         ▼
┌──────────────────────────────────┐
│ BeaconAuthorizationMiddleware │
...
2. **Move role assignment to database** instead of hardcoding in claims transformer:
   ...
   identity.AddClaim(new Claim(BeaconClaims.Role, user.Role));
   ```
...
1. Check `IBeaconUserContext` is registered (done automatically by `AddBeaconUI()`)
2. Verify claims transformer adds `BeaconClaims.UserName`
