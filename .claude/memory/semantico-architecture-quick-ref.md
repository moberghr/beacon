# Semantico Architecture Quick Reference

## 30-Second Overview

Semantico = Database monitoring + Scheduled queries + Multi-channel notifications + ETL orchestration
...
// 2. Add Semantico admin services
builder.Services.AddSemanticoAdmin(builder.Configuration, options =>
{
    options.AddSemanticoScheduler<HangfireScheduler>();
    options.AddEmailAdapter<SmtpEmailAdapter>();
    options.BaseUrl = "https://example.com/semantico";  // For notification links
});
...
{
  "ConnectionStrings": {
    "SemanticoContext": "Host=localhost;Database=semantico;..."
  },
  "Semantico": {
    "BaseUrl": "https://example.com/semantico"
  }
}
```
...
Semantico.UI.AspNet/
└── SemanticoUIExtensions.cs     # UseSemanticoUI() middleware
...
internal sealed class MyCommandHandler : IRequestHandler<MyCommand, MyResponse>
{
    ...
    public MyCommandHandler(SemanticoContext context)
    {
        _context = context;
    }
...
- Schema specified at runtime via `AddPostgreSqlSemantico(..., schema: "name")`
...
### Migrations
```bash
# Schema-agnostic (uses default "semantico")
dotnet ef migrations add MigrationName \
    --project Semantico.Core \
