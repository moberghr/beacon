...
§7.7 **MCP discovery documents** are mapped by `app.MapMcpDiscovery()` and are anonymous BY DESIGN: `/.well-known/oauth-protected-resource`, its `/beacon/mcp` path-inserted variant, and `/.well-known/mcp/server-card.json`. `Beacon.Core.Mcp.McpDiscoveryPaths.AnonymousDiscoveryPaths` is the single source of truth — the routes, `LoginFormAuthMiddleware`, and `FirstRunSetupMiddleware` all derive their allow-list from it. It lives in Core (not MCP) because the middlewares ship in `Beacon.Api`, and Api must not reference the sibling `Beacon.MCP` (§2.4); `McpDiscoveryEndpoints.AnonymousDiscoveryPaths` and `McpDiscoveryDocuments`' path constants are forwarding aliases — do NOT re-declare these paths anywhere. NEVER allow-list `/.well-known` as a prefix; match those exact paths only. Set `Beacon:PublicBaseUrl` in any proxied deployment, or the documents reflect the client-controlled Host header (the host logs a startup warning when it is unset).
