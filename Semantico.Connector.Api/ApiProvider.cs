        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var queryDef = JsonSerializer.Deserialize<ApiQueryDefinition>(query, JsonOptions)
                ?? throw new SemanticoException("Failed to parse API query definition");
    ...
    private ApiConnectionConfig ParseConfiguration(DataSource dataSource)
    {
        try
        {
            ...
            var config = JsonSerializer.Deserialize<ApiConnectionConfig>(decryptedJson, JsonOptions)
                ?? throw new SemanticoException("Failed to parse API connection configuration");
            return config;
        }
        catch (JsonException ex)
        {
            throw new SemanticoException($"Invalid API configuration JSON: {ex.Message}", ex);
        }
    }
}
