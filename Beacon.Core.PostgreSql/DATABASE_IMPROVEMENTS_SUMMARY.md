...
### 1. Backup Database
```bash
pg_dump -h localhost -U postgres -d beacon_db > backup_before_improvements.sql
```
...
**Rollback command:**
```bash
dotnet ef migrations remove --project Beacon.Core.PostgreSql --startup-project ../Beacon.SampleProject
```
