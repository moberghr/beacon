...
/// </summary>
public class AiActorService : IAiActorService
{
    ...
    public async Task<Data.Entities.AiActor> CreateActorAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var dataSource = await context.DataSources
            ...
            ?? throw new SemanticoException($"DataSource with ID {options.DataSourceId} not found");

        var actor = new Data.Entities.AiActor
        {
            Name = options.Name,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var actor = await context.AiActors
            ...
            ?? throw new SemanticoException($"AI Actor with ID {actorId} not found");

        if (actor.Status != AiActorStatus.Active)
        {
            throw new SemanticoException($"AI Actor {actorId} is not active (status: {actor.Status})");
        }
        ...
        try
        {
            ...
            execution.EstimatedCost = llmResponse.EstimatedCost;
    ...
    public async Task PauseActorAsync(int actorId, CancellationToken cancellationToken = default)
    {
        ...
        if (actor.Status != AiActorStatus.Active)
        {
            throw new SemanticoException($"Cannot pause actor with status {actor.Status}");
        }
    ...
    public async Task ResumeActorAsync(int actorId, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new SemanticoException($"Cannot resume actor with status {actor.Status}");
        }
    ...
    public async Task<Data.Entities.AiActor?> GetActorAsync(
        int actorId,
        CancellationToken cancellationToken = default)
    {
        await using var context = await _contextFactory.CreateDbContextAsync(cancellationToken);
    ...
    public async Task<List<Data.Entities.AiActor>> GetActorsForDataSourceAsync(
        int dataSourceId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var actor = await context.AiActors
            ...
            ?? throw new SemanticoException($"AI Actor with ID {options.ActorId} not found");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var plan = await context.AiActorPlans
            ...
            ?? throw new SemanticoException($"Plan with ID {options.PlanId} not found");

        if (plan.Status != AiActorPlanStatus.PendingApproval)
        {
            throw new SemanticoException($"Plan is not pending approval (status: {plan.Status})");
        }
