...
public class ObjectDataReader<TData> : IDataReader
{
    ...
    public ObjectDataReader(IEnumerable<TData> data)
    {
        ...
        var propertyAccessors = typeof(TData)
            ...
            .Where(p => p.CanRead && !p.GetGetMethod().IsVirtual) // filter out virtual properties
            .Select((p, i) => new
            {
                Index = i,
                Property = p,
        ...
        _nameLookup = propertyAccessors.ToDictionary(
            p => p.Index,
            p => p.Property.Name);
    }
    ...
    private Func<TData, object> CreatePropertyAccessor(PropertyInfo p)
    {
        ...
        UnaryExpression castAsObject;

        if (p.PropertyType.IsEnum)
        {
            // If you cast enum to object you get string value of enum
            // so we first cast to int and then to object
            var castToInt = Expression.Convert(propertyAccess, typeof(int));

            castAsObject = Expression.TypeAs(castToInt, typeof(object));
        }
        else
        {
            // Make sure the result of the get method is cast as an object
            castAsObject = Expression.TypeAs(propertyAccess, typeof(object));
        }
