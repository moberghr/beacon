...
/// <summary>
/// Authorization provider that looks up user roles from the Semantico database.
...
public class DatabaseAuthorizationProvider(
    ...
    IUserManagementService userService) : ISemanticoAuthorizationProvider
{
    private SemanticoUserData? _cachedUser;
    ...
    private async Task<SemanticoUserData?> GetCurrentUserAsync(CancellationToken cancellationToken)
    {
        if (_userFetched)
            return _cachedUser;
